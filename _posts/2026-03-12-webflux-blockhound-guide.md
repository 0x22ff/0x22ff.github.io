...
대부분 원인은 비슷하다. **논블로킹 경로 안에 블로킹 코드가 섞여 들어간 경우**다.
...
그리고 그 호출이 논블로킹 스레드에서 일어나면 `BlockingOperationError`를 터뜨린다.

포인트는 성능 최적화 도구라기보다 **위반 탐지 도구**라는 점이다.
